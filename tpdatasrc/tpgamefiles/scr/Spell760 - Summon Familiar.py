...
def GetLevel( npc ):
# Returns characters combined sorcerer and wizard levels - addition by AugustDrake 3/26/23 to add adept to the list 
	level = npc.stat_level_get(stat_level_sorcerer) + npc.stat_level_get(stat_level_wizard) + npc.stat_level_get(stat_level_adept)
	return level
	
# just gonna generalize all this AugustDrake 4/6/23

def Fortitude( npc ):
# Returns Fortitude Save Bonus for all the casters class levels
	bonus = npc.stat_level_get(stat_save_fortitude)
	return bonus

def Reflex( npc ):
# Returns Reflex Save Bonus for all the casters class levels
	bonus = npc.stat_level_get(stat_save_reflexes)
	return bonus

def Will( npc ):
# Returns Will Save Bonus for all the casters class levels
	bonus =  npc.stat_level_get(stat_save_willpower)
	return bonus
